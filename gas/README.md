...
| **U** | `005-Updater.gs`（コードの自動更新・そうさボタン） | U044ver | `005-Updater` |
| **V** | `006-Venue.gs`（会場・イベント情報あつめ） | V025ver | `006-Venue` |
...
| `メール通知 じぶんのアドレス` | リマインダーがメールにも届くようになる（テストが1通飛ぶ） | 公式LINE（個人） |
| `メール通知 解除` | メールを止める | 公式LINE（個人） |
| `メール通知` | いまメールが入っているかを答える | 公式LINE（個人） |
| `katastrophe` ほか | コードを取り込む（**まーくさんのIDのときだけ**） | どこでも |

- **メール通知**は、外出中にLINEが開けないとき・Androidの人向けの受け取り口です。
  ﾃﾞｨｽｺｰﾄﾞは受け取れるようにするまで8手かかりますが、メールは1手で済みます。
