 ...
 *     ★「返事：200」は「ページが開けた」だけで、読めた意味ではありません。
 *       実際に読ませてみて、読めた中身をそのまま出します。
 *       ホームページと目で見くらべられるようにするためです。
 *   ・ℹ️ ニューオータニ・ワントゥワンに、まぎらわしい ⚠️ を出すのをやめた
 *     ★この2つは、一覧のページに日付が出ていません。
 *       催しの名前だけが並び、日にちは中のページにあります。
 *       つまり「一覧に日付が無いのが正しい姿」です。
 ...
 *     ★300人で切ると、ホテルの宴会や小さめのホールが
 *       まるごと落ちていました。100人でも、終わる時刻が重なれば
 *       乗り場は動きます。落とすかどうかを決めるのは走る人です
 *     ★人数が書かれていないものは、これまでどおり落としません
 *
 *  [V045ver]
 ...
 *     ★サーバーは もうあります（西東卍会・4人）。
 *       やることは「送り先（ウェブフック）を1つ作る」だけです
 *     ★もう用意ができているときは、道順ではなく いまの状態を返します
 *   ・🔐 送り先のURLは、公式LINE（1対1）でしか受け取らないようにした
 *     ★あのURLは鍵と同じです。知っている人は、だれでも
 *       そのチャンネルに書き込めます。
 *       グループLINEに貼られたら、そこにいる全員に見えてしまいます
 *     ★グループで送られたときは、覚えずに断り、
 *       「ﾃﾞｨｽｺｰﾄﾞ側で作り直してください」とお伝えします
 *
 *  [V037ver]
 ...
 * これ未満の見込み人数は出さない。
 *
 * ★300人 → 100人にしました（まーくさんのご指示）。
 *   300人で切ると、ホテルの宴会や小さめのホールが
 *   まるごと落ちていました。
 *   100人でも、終わる時刻が重なれば乗り場は動きます。
 *   落とすかどうかを決めるのは、こちらではなく走る人です。
 *   （人数が書かれていないものは、これまでどおり落としません）
 */
const VN_MIN_PEOPLE = 100;
...
function vnProbeOne_(src, now) {
  ...
  if (hits.length) {
    ...
    L.push("　→ 読み取りを作れます");
  } else if (src.deep || src.detail) {
    /*
     * ★ここを直しました（まーくさんからのご質問がきっかけです）。
     *
     *   ニューオータニとワントゥワンは、一覧のページに日付が出ていません。
     *   催しの名前だけが並んでいて、日にちは中のページに書いてあります。
     *   つまり、この2つは「一覧に日付が無いのが正しい姿」です。
     *
     *   それなのに、ここで ⚠️ を出していました。
     *   ちゃんと読めているのに「読めていない」と見えるので、
     *   調べても真偽が分からない、いちばん たちの悪い出方でした。
     */
    L.push("ℹ️ 一覧のページに日付はありません（この会場は、それでふつうです）");
    L.push("　中のページまで開いて読む作りにしてあります。");
  } else {
  ...
  /*
   * ★いちばん知りたいのは「日付が入っているか」ではなく、
   *   「けっきょく、催しが1件でも出てくるのか」です。
   *   なので、実際に読ませてみて、出てきたものをそのまま見せます。
   *   ここが 0件なら、上が ✅ でも読めていません。
   */
  vnProbeTry_(src, now, html).forEach(function (x) { L.push(x); });
  return L;
}

/**
 * ほんとうに催しが出てくるかを、実際に読ませて確かめる。
 *
 * ★「ページは開けました」と「催しが読めました」は、別のことです。
 *   開けただけで安心してしまうと、中身が0件のまま何日も気づけません。
 *   だから、出てきた1件を そのまま画面に出します。
 *   自分の目で、ホームページと見くらべられるようにするためです。
 */
function vnProbeTry_(src, now, html) {
  const L = [];
  // ★中のページまで開く会場は時間がかかるので、見る日数を減らします
  //   （Googleは1回の処理を6分で打ち切ります）
  const days = (src.deep || src.detail) ? 4 : 8;
  const until = Date.now() + 70000;                    // この会場に使ってよいのは70秒まで
  let hit = 0, seen = 0, cut = 0;
  const sample = [];
  const base = now || new Date();
  for (let i = 0; i < days; i++) {
    if (Date.now() > until) { cut = days - i; break; }
    const d = new Date(base.getFullYear(), base.getMonth(), base.getDate() + i);
    let got = [];
    // ★一覧のページは、すぐ上でもう読んであります。
    //   同じページを何度も取りにいくと、先方にも迷惑ですし、時間も食います
    try { got = (vnScrapeOne_(src, d, html || null) || {}).events || []; } catch (e) { got = []; }
    seen++;
    hit += got.length;
    got.forEach(function (e) {
      if (sample.length >= 2) return;
      sample.push("　　" + vnMd_(d) + "　" + String(e.title || "").slice(0, 28) +
                  "　" + (e.start ? (e.startKind || "開始") + e.start : "時刻なし") +
                  (e.end ? "〜" + e.end + (e.endSure ? "" : "（予想）") : ""));
    });
  }
  L.push((hit ? "✅ " : "❌ ") + "実際に読ませてみた結果：" + seen + "日ぶんで " + hit + "件");
  if (cut) L.push("　（時間切れで、残り " + cut + "日ぶんは見ていません）");
  if (sample.length) {
    L.push("　読めた中身（ホームページと見くらべてください）");
    sample.forEach(function (x) { L.push(x); });
  } else if (hit === 0) {
    L.push("　→ 0件です。ページの作りが変わったか、ほんとうに何も無いかの どちらかです。");
    L.push("　　" + src.url);
  }
  return L;
}

/** 「9/21」の形にする（調査の結果を短く出すため） */
function vnMd_(d) {
...
function vnHandleDiscordCmd_(ev) {
  ...
  if (q.kind === "hook") {
    /*
     * ★送り先（ウェブフックのURL）は、鍵と同じあつかいです。
     *   これを知っている人は、だれでもそのチャンネルに書き込めます。
     *   グループLINEに貼られると、そこにいる全員に見えてしまいます。
     *   なので、公式LINE（1対1）で送られたときだけ受け取ります。
     */
    const inGroup = !!(ev && ev.source && (ev.source.groupId || ev.source.roomId));
    if (inGroup) {
      say("⚠️ その送り先は、鍵と同じあつかいです。\n" +
          "グループでは受け取れません。\n" +
          "公式LINE（1対1）のほうで、もう一度送ってください。\n" +
          "※ いま貼ったものは、ﾃﾞｨｽｺｰﾄﾞ側で作り直してください\n" +
          "　（見た人がだれでも書き込めてしまうためです）");
      return true;
    }
    const pr = PropertiesService.getScriptProperties();
  ...
  // 道順（1回だけ・まーくさん）
  let now = "";
  try { now = PropertiesService.getScriptProperties().getProperty("DISCORD_WEBHOOK") || ""; } catch (e) {}
  if (now) {
    say("✅ ﾃﾞｨｽｺｰﾄﾞの用意は、もう終わっています。\n\n" +
        "・送り先　：入っています\n" +
        "・招待リンク：" + (vnDiscordInvite_() ? "入っています" : "まだです（すでに全員入っているなら要りません）") + "\n\n" +
        "入れ直したいときは、新しいURLをこのLINEに送ってください。");
    return true;
  }
  say("💬 ﾃﾞｨｽｺｰﾄﾞの用意（まーくさんが1回だけ）\n\n" +
      "★サーバーがもうあるなら、やることは1つだけです。\n\n" +
      "【送り先（ウェブフック）をつくる】\n" +
      "　① 通知を出したいチャンネル（例：一般）を長押し\n" +
      "　②「編集」→「連携サービス」→「ウェブフック」\n" +
      "　③「新しいウェブフック」を押す\n" +
      "　④ できたものを押して「ウェブフックURLをコピー」\n" +
      "　⑤ そのURLを、この公式LINE（1対1）にそのまま送る\n" +
      "　　（ﾃﾞｨｽｺｰﾄﾞ https://discord.com/api/webhooks/〜）\n\n" +
      "⚠️ そのURLは鍵と同じです。グループには貼らないでください\n" +
      "　（見た人がだれでも書き込めてしまいます）\n\n" +
      "これで終わりです。\n" +
      "すでにサーバーに入っている人は、何も設定せずに通知が届きます。\n" +
      "（まだ入っていない人がいるときは、招待リンクも送ってください。\n" +
      "　ﾃﾞｨｽｺｰﾄﾞ https://discord.gg/〜）");
  return true;
}
