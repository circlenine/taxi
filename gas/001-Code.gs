 ...
 *  ★★★  C053ver  （2026/09/17）  ★★★   ← もとは version 232
 *
 ...
 *
 *  [C053ver]
 *   ・LINEの合図に「アニメに名言変更してください」を足した（updHandleQuote_）
 *     終わったときに添えるひとことを、英語 ⇔ アニメ で切り替えます
 *
 *  [C052ver]
 ...
 *     新しいコードが出たら、こちらで気づいて取り込みます
 *
 *  [C050ver]
 *   ・設定タブ「コードの置き場（GitHub）」の説明を直した
 *     空でも大丈夫（005-Updater が心当たりをさがす）ことを書いた
 *
 *  [C049ver]
 *   ・LINEの合図に「おきば」を足した（005-Updater の updHandleRepo_ を呼ぶ）
 *
 *  [C048ver]
 ...
 *     ★これだけは、1文字でも違えば動きません。大文字・小文字もそのままです
 *     ★ふだんは打つ必要がありません。コードを入れ替えたときに、
 *       見張りは自動でそろいます。これは「それでもおかしいとき」用です
 *     ★前からの「なおして」も、そのまま残してあります
 *
 *  [C046ver]
 *   ・合言葉「katastrophe」を受けるようにした（updHandleKata_）
 *     公式LINEでもグループLINEでも効く。まーくさん以外は動かない
 *
 *  [C045ver]
 ...
 *  [C041ver]
 *   ・読めなかったときの一言を
 *     「死神の目と契約しない限り、解決方法がわかりません」にした
 *   ・「人間って…面白ォオー!!!!」に直した
 *
 *  [C040ver]
 *   ・LINEの返事を、叫ぶ系のデスノートネタにそろえた
 *     計★画★通★り／くそっ!!!!やられた!!!!／わ…私は仰せの通りに…／
 *     貴方が神か…／わけがわからない…／死神の目をもってしても／
 *     人間って…面白ォオー!!!!（1回に10件以上そろったとき）
 *     ★命令口調にはしない。うまくいかなかったときの叫びは、
 *       相手にではなく、こちら（読み取る側）に向ける
 *
 *  [C039ver]
 ...
 *     みんなが雑談で普通の写真を送ることがある。そこに毎回
 *     「読み取れませんでした」と返していたので、ただの邪魔になっていた。
 *     1件も写っていない＝乗車記録の写真ではない、とみなして黙る。
 *     逆に、うまくいったときは1枚でも「〇件を取り込みました」と返すようにした。
 *     1件でも読めていれば不備も今までどおり伝える
 *     （ここで黙ると、落ちた記録に誰も気づけなくなるため）
 *
 *  [C029ver]
 *   ・ホテルの予定表の写真を、オプチャのスクショと取り違えないようにした
 *     いままでは「画像＝オプチャ」と決め打ちだったので、帝国ホテルや
 *     リーガロイヤルの資料を送ると、乗車記録として書き込まれてしまっていた。
 *     二段構えで分ける：
 *       ①「ホテル」と打ってから写真を送ると、ホテルの予定表として読む
 *         （先に写真を送ってしまったときは「↑ホテル」で読み直す）
 *       ② 合図が無くても、乗車記録が1件も読めなかったときだけ
 *         ホテルの予定表として読み直す。ふつうのオプチャ画像には触れない
 *     読み取った予定は、当日17:00のイベント案内に出る（006-Venue.gs）
 *
 *  [C028ver]
 *   ・設定に「コードの置き場（GitHub）」「コードの枝（ブランチ）」「コードのフォルダ」を足した
 *     スマホだけでコードの更新先を直せるようにするため
 *     （鍵＝トークンは他人に見られては困るので、シートには置きません）
 *
 *  [C027ver]
 ...
 *     ・自動送信する日（毎月）… 既定16日
 *     ・自動送信の時刻（時）　… 既定7時
 *   ・設定に「まとめスプシを同じ人に見せる」を足した
 *     記録用スプシを触れる人に、まとめスプシの「見るだけ」を自動で付ける
 *
 *  [C025ver]
 *   ・設定に「まとめスプシのID」を足した
 *     レポートのダッシュボードを、自分で決めたスプレッドシートに作れるようにする
 *     （URLをそのまま貼ってよい。スマホからでも変えられる）
 *
 *  [C024ver]
...
const SETTINGS_DEFS = [
  ...
  { key: "コードを自動で取り込む",     def: "はい", kind: "yn",
    help: "新しいコードが出たら、こちらで気づいて取り込みます（15分おきに1回だけ見ます）。"
        + "止めたいときは「いいえ」" },
  { key: "コードの置き場（GitHub）",   def: "",   kind: "text",
    help: "コードを取ってくる場所。空でも大丈夫です（こちらで心当たりをさがします）。※鍵（トークン）はここには置きません" },
  { key: "コードの枝（ブランチ）",     def: "",   kind: "text",
    help: "空なら、GitHubに既定の枝を聞いて自動で決めます。変えたいときだけ書いてください" },
  { key: "コードのフォルダ",           def: "gas", kind: "text",
    help: "置き場の中の、どのフォルダを見るか" },
  { key: "LINEに出す時間帯の数",       def: 3,    kind: "num",
    help: "LINEの絵に、曜日区分ごとに何個の時間帯を出すか。" +
          "少ないほど1通に収まりやすい。全部の時間帯はまとめスプシで見られます" },
  { key: "レポートを自動で送る",       def: "はい", kind: "text",
  ...
  { key: "イベント情報を自動で送る",   def: "いいえ", kind: "text",
    help: "「はい」にすると、その日にイベントがある日だけ、16:30にまーくさんへ確認用を送り、17:00にグループLINEへ送ります。" +
          "何も無い日は送りません。はじめは「いいえ」（切）にしてあります" },
  { key: "まとめスプシを同じ人に見せる", def: "はい", kind: "text",
    help: "記録用スプシを触れる人に、まとめスプシの「見るだけ」の権限を自動で付けます" },
  { key: "まとめスプシのID", def: "", kind: "text",
    help: "レポートのダッシュボードを作る先。スプレッドシートのURLをそのまま貼ってOK。" +
          "空なら、説明タブZ2に書いてあるものを使います" },
...
function handleEvent_(ev) {
  ...
  // --- 「おきば」… どこの置き場を読むかを、LINEから決める（まーくさんだけ）---
   ...
   *     英語の名言 ⇔ ジャンプ作品の名言・迷言
   */
  if (typeof updHandleQuote_ === "function" && updHandleQuote_(ev)) return;
...
function rememberNewSender_(ev, userId) {
  try {
    ...
    if (!already) {
      lineReply_(ev.replyToken || "",
        "👋 な…なんだと…!?" + (who ? "　" + who + "さん、" : "") + "はじめまして。\n" +
        "「" + UNKNOWN_TAB + "」の席は、もう埋まっています。記録はいったんそこへ。\n" +
        "空き席が決まったら「〇〇登録」と打ってください。");
    }
...
function handleSenderRegister_(ev) {
  ...
  const userId = (ev.source && ev.source.userId) || "";
  if (!userId) { lineReply_(ev.replyToken || "", "うわあああ!!!! IDが取れない!!!!"); return true; }
  senderLearn_(userId, want);
  lineReply_(ev.replyToken || "",
    "✅ わ…私は仰せの通りに…\n以後あなたの記録は「" + want + "」タブへ。計★画★通★り");
  return true;
}
 ...
 * ★見張りを立て直す合言葉。
 *
 *   「Kataskatrophe」だけは、打った字と1文字でも違えば動きません。
 *   大文字・小文字も、そのままでないと通りません。
 *   ほかの合言葉（katastrophe など）は、ひらがなでも飾り文字でも
 *   通るようにしてありますが、こちらは わざと厳しくしてあります。
 *   ★ふだんは、打つ必要そのものがありません。
 *     コードを入れ替えたときに、見張りはこちらで自動でそろえます。
 *     これは「それでもおかしいとき」の手元のスイッチです。
 */
const REPAIR_WORD = "Kataskatrophe";

function handleRepairNote_(ev) {
  const raw = String((ev.message && ev.message.text) || "").trim();
  const t = raw.replace(/[\s\u3000]/g, "");
  // ★まず、1文字も違わない「Kataskatrophe」だけを見る
  if (raw !== REPAIR_WORD && t !== REPAIR_WORD &&
      !/^(なおして|直して|なおして！|直して！|修復|リセット)$/.test(t)) return false;
  ...
  lineReply_(ev.replyToken || "", "🔧 早くなんとかしないと…\n\n" + text);
  return true;
}
...
function handleDateNote_(ev, note) {
  ...
  if (!quoted && note.dir === "next") {
    ...
    lineReply_(reply, "📅 わ…私は仰せの通りに…　次のスクショは " + label + " で");
    return;
  }
  ...
  if (!mid) {
    ...
    cache.put("PENDDATE_" + uid, dateToYmd_(note.bizDate), 3600);
    lineReply_(reply, "📅 直前のスクショが見当たらない…　次のぶんを " + label + " で控えました");
    return;
  }

  let n = 0;
  try { n = fixOpuchaDate_(mid, note.bizDate); }
  catch (e) { logErr_("fixOpuchaDate", e); lineReply_(reply, "❌ くそっ!!!!やられた!!!!\n" + e.message); return; }

  if (!n) {
    lineReply_(reply, "📅 だ…ダメだ…直す行が見当たらない…（未取込か、消えた行かもしれません）");
    return;
  }
  lineReply_(reply, "📅 " + label + " に直しました（" + n + "件）。計★画★通★り");
}
 ...
 * 返信の文面を作る。
 *
 * ★返すのは「乗車記録として読めたとき」だけ。
 *   みんなが雑談で普通の写真を送ることがある。そこに毎回
 *   「読み取れませんでした」と返すと、ただの邪魔になる。
 *   1件も写っていない＝乗車記録の写真ではない、とみなして黙る。
 *
 *   ただし1件でも読めていれば、必ず何か返す。
 *   ・全部うまくいった → 「〇件を取り込みました」と伝える（1枚でも伝える）
 *   ・不備があった     → 何枚目の何がダメだったかを書く
 *     ここで黙ってしまうと、落ちた記録に誰も気づけなくなる。
 */
function opuchaReplyText_(who, arr, total) {
  const sama = who ? String(who).trim() + "様" : "";
  let ok = 0, read = 0;
  arr.forEach(function (x) { ok += x.ok; read += (x.read || 0); });
  const bad = arr.filter(function (x) { return x.ng && x.ng.length; });

  // 1件も写っていない ＝ 雑談の写真。何も言わない
  if (read === 0) return "";
  ...
  if (!bad.length) {
    const head = (total <= 1)
      ? "📷 " + sama + ok + "件、取り込みました。計★画★通★り"
      : "📷 " + sama + "スクショ" + total + "枚（合計" + ok + "件）" +
        "／ジェバンニが一晩でやってくれました" +
        (ok >= 10 ? "\n人間って…面白ォオー!!!!" : "");
  ...
  const lines = [];
  lines.push("📷 " + (sama ? sama + "データ…" : "いただいたデータ…") + "くそっ!!!!やられた!!!!");
  lines.push("死神の目と契約しない限り、解決方法がわかりません。");
  lines.push("");
  ...
  lines.push("時刻・金額・乗り場さえ写っていれば、次は僕の勝ちだ。");
  return lines.join("\n");
}
