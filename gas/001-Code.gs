 ...
 *     ★ご指摘「特に何も反応がありません」。
 *       絵文字ひとつで呼べるようにしたぶん、似ている別の絵文字
 *       （☀️のつもりで🌞など）を送ってしまうことが必ず起きます。
 *       そのとき黙っていると「こわれている」としか見えません
 *     ★まーくさんが記号だけを送ったときにかぎり、一覧をそえて返します
 *
 *  [C057ver]
 ...
 *     切ってあるせいで、イベントの案内が1回も動いていませんでした
 *   ・その説明の時刻も直した（前日18:00 確認用 → 当日17:00 グループ）
 *
 *  [C054ver]
 *   ・設定に「レポートの1行の文字数」「イベント案内の1行の文字数」を足した
 *     どちらも空のままで構いません。
 *     言葉のまん中で切れるのが気になるときだけ、数字を入れてください
 *
 *  [C053ver]
...
const SETTINGS_DEFS = [
  /*
   * ★1行の文字数。どちらも、空のままで構いません。
   *   ・まとめスプシは、らんの幅から こちらで逆算しています（設定は要りません）
   *   ・LINEの絵は、見る人の機種と文字の大きさで幅が変わるので測れません。
   *     言葉のまん中で切れるのが気になるときだけ、数字を入れてください
   */
  { key: "レポートの1行の文字数",     def: "",     kind: "text",
    help: "空ならLINEにおまかせ。数字（20〜60）を入れると、その幅で句読点のところで改行します" },
  { key: "イベント案内の1行の文字数", def: "",     kind: "text",
    help: "空なら既定（23）。数字（20〜40）を入れると、注意書きがその幅で詰まります" },
  ...
  { key: "イベントのお知らせは何分前", def: 60,   kind: "num",
    help: "ボタンで「お知らせ」を選んだとき、催しが終わる何分前に知らせるか。" +
          "5〜300で指定します（既定は60分前）" },
  { key: "イベント情報を自動で送る",   def: "はい", kind: "text",
    help: "前の日の18:00に まーくさんへ「あすのぶん」の確認用を送り、" +
          "当日の17:00に グループLINEへ送ります。" +
          "何も無い日は送りません。止めたいときは「いいえ」にしてください" },
...
function handleEvent_(ev) {
  ...
  if (!rec) {
    /*
     * ★絵文字だけを送ったのに、どれにも当たらなかったとき。
     *
     *   こちらからの見直しで足しました。
     *   絵文字ひとつで呼べるようにしたぶん、打ちまちがい
     *   （似ているけれど別の絵文字）が必ず起きます。
     *   そのとき黙っていると、打った人には
     *   「こわれている」としか見えません。
     *   実際、☀️のつもりで🌞を送って、何も返らないことがありました。
     *
     *   まーくさんが短い記号だけを送ったときにかぎり、
     *   「その絵文字は割り当てていません」と、一覧をそえて返します。
     */
    try { emojiMiss_(ev); } catch (e) { logErr_("emojiMiss", e); }
    return;              // 記録として読めないものは雑談。何もしない
  }
...
/**
 * 絵文字だけ送られて、どれにも当たらなかったときの受け止め。
 *
 * ★まーくさん以外には、何も返しません（グループが うるさくなるため）。
 * ★ふつうの文には反応しません。
 *   「字が1〜3文字ぶんで、ひらがな・カタカナ・漢字・英数字を含まないもの」
 *   ＝ ほぼ絵文字か記号だけ、のときにかぎります。
 */
function emojiMiss_(ev) {
  const raw = String((ev.message && ev.message.text) || "").trim();
  if (!raw) return false;
  // 字が入っているものは、雑談かもしれないので何もしない
  if (/[ぁ-んァ-ヶ一-龥A-Za-z0-9]/.test(raw)) return false;
  // 長いものも、何もしない（記号の並びは、ふざけて送られることがある）
  ...
  lineReply_(ev.replyToken || "",
    ...
    "💩 コードの取り込み\n\n" +
    "※ 一覧は「❓」でも出せます。\n" +
    "※ この絵文字を使いたいときは、そう言ってください。割り当てます。");
  return true;
}
