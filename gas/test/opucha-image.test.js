...
has(F('opuchaReplyText_')('ダイスケ', [{ idx: 1, read: 2, ok: 2, ng: [] }], 1),
    '2件、取り込みました。計★画★通★り', '1枚でもうまくいったら、件数を返す');
...
console.log('\n■ 知らないIDから記録が届いたら、ｼﾞﾝタブへ入れる');
{
  ...
  has(sent[0], '仰せの通りに', '  そう返事する');
  has(sent[0], '計★画★通★り', '  ユーモアも入れる');
...
console.log('\n■ 知らないIDの1人目は、自動で ｼﾞﾝ タブの人になる');
{
  ...
  has(sent[0], 'ｼﾞﾝ', '  どのタブに入るかだけ伝える');
  has(sent[0], 'ワタリが席を用意しました', '  ユーモアも入れる');
  has(sent[0], '貴方が神か', '  叫ぶ系のネタも入れる');
  ok(sent[0].indexOf('登録') === -1, '  「登録と打ってください」とは言わない');
  ok(sent[0].split('\n').length <= 2, '  2行までの、端的な文にする');
  ...
  has(sent[0], '席は、もう埋まっています', '  その理由をはっきり伝える');
