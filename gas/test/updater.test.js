...
console.log('\n■ 自動の知らせの2行目に、そのバージョンが出る');
{
  ...
  t(lines[1] === '059 Code' && lines[7] === '003 Tenki',
  ...
  T('スプシ',
    '入れ替え：001-Code\nバージョン：C059/U096\n見張り：ぜんぶ そろっています');
  const z = String(ctx.pu[0].msgs[0].text);
  t(z.indexOf('059 Code') !== -1 && z.indexOf('096 Updater') !== -1,
    '★手で打ったときも、バージョンを出す', z);
  ...
  // ★記号（U）は外して出すので、数字のところだけで見ます
  t(noV.indexOf(vm.runInContext('UPD_VERSION', ctx).replace(/^[A-Za-z]+|ver$/g, '')) !== -1,
