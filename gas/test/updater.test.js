...
t(items.length === 10, 'いつも10個並ぶ（' + items.length + '個）');
...
t(F('panelReadRows_')(panel).every(r => r.value === false),
  '足した行にチェックボックスが付く');
has(alerts[alerts.length - 1].b, '6個のボタンを足しました', '何個足したか伝える');
...
t(F('panelCheck_')(panel).missing.length === 4, '[7]〜[10] が足りない');
// 5つめ6つめを消して、足りない状態を作る
delete panel._cells['44,2']; delete panel._cells['44,3'];
delete panel._cells['46,2']; delete panel._cells['46,3'];
const miss = F('panelCheck_')(panel).missing;
t(miss.length === 6, '6つ足りない');
F('menuMakePanel')();
t(F('panelCheck_')(panel).missing.length === 0, '足したのでそろった');
t(F('panelReadRows_')(panel).length === 10, '10個になった');
...
console.log('\n■ コードを更新したら、増えたボタンを自分で足す');
{
  ...
  t(F('panelCheck_')(panel).missing.length === 4, '[7]〜[10] が無い状態');
...
console.log('\n■ バージョン');
t(vm.runInContext('UPD_VERSION', ctx) === 'U013ver', 'U013ver になっている');
reset([['001-Code.gs', 'あたらしい']]);
F('menuUpdateStatus')();
has(alerts[0].b, 'U013ver', '状態画面にバージョンが出る');
...
console.log('\n■ 推定残り時間を出す');
{
  ...
  t(items.every(x => x.sec > 0), '10個とも見込み時間を持っている');
}
