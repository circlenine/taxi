...
console.log('\n■ ふきだしで囲む');
{
  ...
  const b = B('🚕 ワンメーター星人');
  const ls = b.split('\n');
  t(ls.length === 3, '★3行だけ（上のふち・中身・下のふち）');
  t(ls[0] === '╭───────────────╮', '上のふち');
  t(ls[1].indexOf('ワンメーター星人') !== -1, '  中身は、そのまま1行');
  t(ls[1].indexOf('┃') === -1, '★中身に、ふちの記号を付けない（行頭がそろって読みにくくなるため）');
  t(ls[2].indexOf('⌄') !== -1, '下のふちの真ん中に、しっぽ');
  t(B('').split('\n').length === 3, '中身が空でも形はくずれない');
  t(B(null).split('\n')[0].indexOf('╭') === 0, 'null でも落ちない');
  t(ls[0].length === ls[2].length, '★上と下のふちは、同じ長さ');

  // 星人の紹介は「見出しだけ囲って、中身は外に出す」
  const card = F('updAlienBlock_')();
  t(card.indexOf('この方を　乗車させて来て下ちい') !== -1, '見出しの言い回しは そのまま');
  t(card.indexOf('╭───') !== -1, '★名前は ふきだしの中');
  t(card.indexOf('🔸') !== -1, '★特徴は しるし付きで、ふきだしの外');
  t(card.indexOf('💛 好きなもの') !== -1, '  好きなものも');
  t(card.indexOf('💢 きらいなもの') !== -1, '  きらいなものも');
  t(card.indexOf('💬 口ぐせ') !== -1, '  口ぐせも');
  t(card.indexOf('⭐ とくてん') !== -1, '  とくてんも');
  t(card.indexOf('┃') === -1, '★ふちの記号は、どこにも残っていない');
  t(card.indexOf('▚') === -1, '  かすれた四角も使わない');
}
