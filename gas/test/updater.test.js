...
console.log('\n■ 自動の知らせの2行目に、そのバージョンが出る');
{
   ...
   *   入れ替わったものだけに付けます。
   */
  t(st[1] === 'C059', '　変わっていないものには、何も付けない');
  t(st[2] === 'L059　←NEW!!', '★★変わったものに ←NEW!! を付ける', st.join('／'));
  t(st[3] === 'U096', '　残りも、そのまま');
 ...
 */
{
  ...
  t(V('C065/E005/*U121') === 'C065\nE005\nU121　←NEW!!',
    '★★／で分けず、改行して並べる', JSON.stringify(V('C065/E005/*U121')));
  t(V('*U121') === 'U121　←NEW!!', '★変わったものには ←NEW!!');
  t(V('U121') === 'U121', '　変わっていないものには、何も付けない');
  t(V('C065/E005').indexOf('*') === -1, '★「*」は、もう出さない');
  t(V('') === '', '空でも落ちない');
  t(V(null) === '', 'null でも落ちない');
  t(V('C065//E005') === 'C065\nE005', '　余分な／があっても、空の行を作らない');
}
