...
console.log('\n■ 自動の「とりこみ かんりょう」は、1行だけにする（ご指示）');
{
  ...
  t(z.indexOf('C064') !== -1 && z.indexOf('U114　←NEW!!') !== -1,
    '★バージョンは、1行に1つ。変わったものに ←NEW!!', z);
  /*
   * ★ここだけは、縦長になってもかまいません（ご指示）。
   *   ほかの知らせは5行までですが、
   *   バージョンは「どれが入れ替わったか」がすべてなので、
   *   1行に1つ並べます。
   */
  t(z.split('\n').length === 3,
    '★見出し1行＋バージョン2行（' + z.split('\n').length + '行）', z);
...
console.log('\n■ 自動の知らせの2行目に、そのバージョンが出る');
{
  ...
  const lines = x.split('\n');
  /*
   * ★バージョンは、1行に1つ並べます（まーくさんのご指示）。
   *   「C059/E005/L058/…」は、走りながらでは目で追えません。
   *   ここだけは縦長になってもかまいません。
   *   どれが入れ替わったのかが、ぱっと分かることのほうが大事です。
   */
  t(lines.length === 8, '★1行目＝かんりょう、そのあと1行に1つ（' + lines.length + '行）', x);
  t(lines[1] === '001-Code C059' && lines[7] === '007-Tenki T003',
    '★★／で分けず、1行に1つ並べる。名前も添える', x);
  t(x.indexOf('/') === -1, '★／は、もう使わない', x);
  ...
  t(st.length === 4, '★説明の行までは、LINEに持ってこない（' + st.length + '行）', st.join('／'));
  /*
   * ★「*」ではなく「←NEW!!」にします（ご指示）。
   *   記号だけでは、何のことか分かりません。
   *   入れ替わったものだけに付けます。
   */
  t(st[1] === '001-Code C059', '　変わっていないものには、何も付けない');
  t(st[2] === '003-LineReport L059　←NEW!!',
    '★★変わったものに ←NEW!! を付ける', st.join('／'));
  t(st[3] === '005-Updater U096', '　残りも、そのまま');
  ...
  T('スプシ',
    '入れ替え：001-Code\nバージョン：C059/U096\n見張り：ぜんぶ そろっています');
  const z = String(ctx.pu[0].msgs[0].text);
  t(z.indexOf('C059') !== -1 && z.indexOf('U096') !== -1,
    '★手で打ったときも、バージョンを出す', z);
 ...
 */
{
  ...
  t(m.indexOf('U098　←NEW!!') !== -1, '  入ったバージョンは、ちゃんと出す', m);
  ctx.pu.length = 0;
}
 ...
 */
{
  const V = F('updVersLines_');
  /*
   * ★記号だけでは、どのファイルのことか思い出せません（ご指示）。
   *   「ここだけは略称せずに、正確な英語名を入れておいてください」
   */
  t(V('C065/E005/*U121') ===
    '001-Code C065\n002-Extras E005\n005-Updater U121　←NEW!!',
    '★★／で分けず、改行して並べ、スクリプト名も添える',
    JSON.stringify(V('C065/E005/*U121')));
  t(V('*U121') === '005-Updater U121　←NEW!!', '★変わったものには ←NEW!!');
  t(V('U121') === '005-Updater U121', '　変わっていないものには、何も付けない');
  t(V('C065/E005').indexOf('*') === -1, '★「*」は、もう出さない');

  // ★9つぜんぶ、名前に戻せること（1つでも抜けると、そこだけ記号のまま出る）
  const NAMES = vm.runInContext('UPD_VERS_FILE', ctx);
  t(['C', 'E', 'L', 'W', 'U', 'V', 'T', 'M', 'S'].every(k => !!NAMES[k]),
    '★★9つのファイルぜんぶ、名前に戻せる', JSON.stringify(NAMES));
