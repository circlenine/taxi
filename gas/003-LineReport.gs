 ...
 *  [L062ver]
 *   ・🗺 タブ名を「地図」にそろえた（ご指摘）
 *     ★タブ名は「絵文字なし・全角2文字以内」という決まりをいただいて
 *       いたのに、こちらは「🗺️乗り場マップ」のままでした。
 *       まーくさんが手で「地図」に変えてくださっていたのに、
 *       コードが追いかけていませんでした。
 *     ★前の名前でも見つけられるよう、控えとして残してあります。
 *
 *  [L061ver]
 ...
 *   タブが多いので、長い名前や絵文字が混ざると、
 *   スマホではタブの行がすぐ埋まって、目当てのものを探せません。
 *   前は「🗺️乗り場マップ」でした。決まりを作っていただいたのに、
 *   こちらが直していませんでした。まーくさんが手で「地図」に
 *   変えてくださっていたのに、コードは古い名前のままでした。
 */
const MAP_TAB  = "地図";
/** 前の名前。まだ変えていない人のスプシでも、見つけられるように残す */
const MAP_TAB_OLD = ["🗺️乗り場マップ", "乗り場マップ", "マップ", "map"];
...
function mapFindSheet_(ss) {
  ...
  try {
    if (typeof mnFindSheet_ === "function") {
      const got = mnFindSheet_(MAP_TAB, MAP_TAB_OLD);
      if (got) return got;
    }
  } catch (e) {}
  if (!ss) return null;
